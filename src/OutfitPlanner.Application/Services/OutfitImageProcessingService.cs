...
public class OutfitImageProcessingService : IOutfitImageProcessingService
{
    ...
    {
        // Step 0: Remove background if service is available
        Stream processedStream = imageStream;
        if (_backgroundRemovalService?.IsConfigured == true)
        {
    ...
    public async Task<List<(ProcessedClothingItem Item, Point Position)>> ProcessOutfitItemsAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var processedItems = new List<ProcessedClothingItem>();
        
        // Process each item
        foreach (var (imageStream, clothingType, name) in items)
        {
            ...
            // Reset stream position again for processing
            imageStream.Position = 0;
            
            // Process the image (includes background removal if configured)
            var processedImage = await ProcessClothingItemAsync(imageStream, clothingType, cancellationToken);
            var boundingBox = DetectBoundingBox(originalImage);
            
            processedItems.Add(new ProcessedClothingItem
            {
